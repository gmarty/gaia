/* global Bluetooth, IACHandler, AppWindowManager */
/* exported MusicModule */

'use strict';

/**
 * Music module of Settings Drawer.
 */

function MusicModule(container, options) {
  this.container = container;
  this.nowPlaying = container.querySelector('.media-playback-nowplaying');
  this.controls = container.querySelector('.media-playback-controls');

  this.description = container.querySelector('.description');

  this.rewindButton = container.querySelector('.rewind');
  this.playPauseButton = container.querySelector('.play');
  this.forwardButton = container.querySelector('.forward');

  this.container.addEventListener('click', this);
  window.addEventListener('iac-mediacomms', this.handleMessage.bind(this));
  // When SCO status changes, we need to adjust the ui of the playback controls
  window.addEventListener(
    'bluetoothprofileconnectionchange', this.handleSCOChange.bind(this)
  );

  if (options && options.nowPlayingAction === 'openapp') {
    this.nowPlaying.addEventListener('click', this.openMediaApp.bind(this));
  }

  // Listen for when the music app is terminated. We know which app to look
  // for because we got it from the "appinfo" message. Then we hide the Now
  // Playing container. XXX: This is a gigantic hack, stemming from
  // <https://bugzilla.mozilla.org/show_bug.cgi?id=915880>. If you're thinking
  // about doing something similar, step away from your keyboard immediately.
  window.addEventListener('appterminated', function(event) {
    if (event.detail.origin === this.origin) {
      this.hidden = true;
    }
  }.bind(this));
}

MusicModule.prototype = {
  get hidden() {
    return this.container.hidden;
  },

  set hidden(value) {
    this.container.hidden = value;
    return value;
  },

  handleMessage: function mm_handleMessage(event) {
    var message = event.detail;
    switch (message.type) {
      case 'appinfo':
        this.updateAppInfo(message.data);
        break;
      case 'nowplaying':
        this.updateNowPlaying(message.data);
        break;
      case 'status':
        this.updatePlaybackStatus(message.data);
        break;
    }
  },

  handleSCOChange: function mm_handleSCOChange(event) {
    var name = event.detail.name;
    var connected = event.detail.connected;

    if (name === Bluetooth.Profiles.SCO) {
      this.container.classList.toggle('disabled', connected);
    }
  },

  updateAppInfo: function mm_updateAppInfo(info) {
    if (!info) {
      return;
    }

    this.origin = info.origin;
  },

  updateNowPlaying: function mm_updateNowPlaying(metadata) {
    if (!metadata) {
      return;
    }

    var content = [];
    var title = metadata.title && String(metadata.title).trim();
    var artist = metadata.artist && String(metadata.artist).trim();

    if (title) {
      content.push(title);
    }
    if (artist) {
      content.push(artist);
    }

    content = content.join(' - ');
    this.description.textContent = content;
  },

  updatePlaybackStatus: function mm_updatePlaybackStatus(status) {
    switch (status.playStatus) {
      case 'PLAYING':
        this.hidden = false;
        this.playPauseButton.classList.remove('paused');
        break;
      case 'PAUSED':
        this.hidden = false;
        this.playPauseButton.classList.add('paused');
        break;
      case 'STOPPED':
      case 'mozinterruptbegin':
        this.hidden = true;
        break;
    }
  },

  openMediaApp: function mm_openMediaApp(event) {
    if (this.origin) {
      var evt = new CustomEvent('displayapp', {
        bubbles: true,
        cancelable: true,
        detail: AppWindowManager.getApp(this.origin)
      });
      window.dispatchEvent(evt);
    }
  },

  handleEvent: function mm_handleEvent(event) {
    var port = IACHandler.getPort('mediacomms');
    if (!port) {
      return;
    }

    var target = event.target;
    var command = null;

    switch (target) {
      case this.rewindButton:
        command = 'prevtrack';
        break;
      case this.playPauseButton:
        // The play/pause indicator will get set once the music app replies with
        // its "mode" message, but this will make us appear speedier.
        target.classList.toggle('paused');
        command = 'playpause';
        break;
      case this.forwardButton:
        command = 'nexttrack';
        break;
    }

    if (command) {
      port.postMessage({command: command});
    }
  }
};
